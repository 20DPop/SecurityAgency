...
export default function ProcesVerbalPredarePrimire() {
  ...
  const [formData, setFormData] = useState({
    data_incheierii: new Date().toISOString().slice(0, 16), // Format pentru datetime-local
    nume_sef_formatie: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:3000/api/proces-verbal-predare/create', payload, config);

      alert('✅ Proces verbal de predare-primire salvat cu succes!');
      navigate(-1); // Navighează la pagina anterioară (Pontare)
    } catch (err) {
    ...
    <div className="pv-predare-container">
      <form onSubmit={handleSubmit} className="pv-predare-form">
        ...
        <div className="form-group">
          ...
          <textarea id="obiecte_predate" name="obiecte_predate" value={formData.obiecte_predate} onChange={handleChange} rows="6" placeholder="Descrieți pe scurt ce se predă (chei, documente, observații despre tură, etc.)" required></textarea>
        </div>
        ...
        <div className="form-actions">
          ...
          <button type="submit" className="submit-btn" disabled={loading}>
            {loading ? 'Se salvează...' : 'Salvează Proces Verbal'}
          </button>
        </div>
      </form>
    </div>
